...
object MyListDemo extends MappersImpl with MyListImpl {
  def multiplyList[L[_]: Mapper](list: L[Int]) = {
    implicitly[Mapper[L]].map(list)(_ * 2)
  }
  def doSomething[L[_]: Mapper](l: L[Int]) = Mapper[L].map(l)(_ * 5)

  def multiplyListDouble[L[_]](list: L[Int])(implicit mapper: Mapper[L]) = {
    mapper.map(list)(_ * 3.7)
  }
  def divisionList[L[_]: Mapper](list: L[Int]) = {
    implicitly[Mapper[L]].map(list)(_ / 10.11)
  }
